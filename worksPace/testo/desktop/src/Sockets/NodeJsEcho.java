...
public class NodeJsEcho extends Thread {
	...
	public static void main(String[] args) throws UnknownHostException, IOException, ClassNotFoundException {
		...
		NodeJsEcho.serverSocket = new DatagramSocket(Constantes.portServer);
		m.start();
		ServerVisual s = new ServerVisual();
		s.frmElMejorServer.setVisible(true);
		ServerVisual.print("Puerto: " + NodeJsEcho.serverSocket.getLocalPort());
						...
						if (c.getUsuario() != null) {
							for (Cliente c2 : clientes) {
								if (c2.getUsuario() != null && c2.getUsuario().getP() != null) {
									...
									try {
										c.broadCast("/syncro " + pBody.getX() + " " + pBody.getY() + " "
												+ c2.getUsuario().getId() + " " + pBody.getVelx() + " "
												+ pBody.getVely());
										c.enviarUdp(("/syncro " + pBody.getX() + " " + pBody.getY() + " "
												+ c2.getUsuario().getId() + " " + pBody.getVelx() + " "
												+ pBody.getVely()), c.getIp());
