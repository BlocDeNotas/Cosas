...
public class Player extends ObjetoFisico implements MetodosFisicos {
	...
	public void updateAnimacion() {
		...
		for (Integer input : Juego.controlesSinLoop) {
			for (Integer input2 : teclasPulsadas) {
				if(input == input2) loop = false;
			}
		}
		p.setAnimLoop(loop);
		if(p.getBody().getVely()>0) {
			//if(p.getBody().getY()<Juego.ySuelo+100) p.setAnimActual(Player.animaciones.get("iniciarSalto").getAnimacion());
			p.setAnimActual(Player.animaciones.get("rodarAire").getAnimacion());
		}
		else if(p.getBody().getVely()<0) p.setAnimActual(Player.animaciones.get("caer").getAnimacion());
		else if(!loop) {
			...
			if(teclasPulsadas.contains(Input.Keys.F)) {
				...
				switch(p.getContCombo()) {
					...
					case 1: 
						...
						p.setAnimActual(Player.animaciones.get("ataque2").getAnimacion()); break;
					case 2: 
						if(p.getAnimActual()!= Player.animaciones.get("ataque3").getAnimacion()) {
							p.setTimerAnim(0);
							//p.getAtaques().add(new Ataque(10, 0, 0,p.getBody().getX(),50));
						}
						p.setAnimActual(Player.animaciones.get("ataque3").getAnimacion()); break;
				}
		...
		if(p.getBody().getVelx()>0) {
			p.setDir(true);
		} else if(p.getBody().getVelx()<0)p.setDir(false);
		
		
		if(p.getComboEspadaTimer()>0) p.setComboEspadaTimer(p.getComboEspadaTimer()-1);
	...
	public static Player buscarPlayer(int id) {
		...
		for (Player temp : Juego.players) {
			if(temp.getId()==id)p = temp;
		}
		if(p == null) System.out.println("Player no encontrado");
		return p;
	}
