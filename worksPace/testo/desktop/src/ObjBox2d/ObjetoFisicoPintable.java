...
public abstract class ObjetoFisicoPintable {
	...
	public void pintar(boolean debug) {
		...
		try {
			if (animActual.getKeyFrames().length > 1)
				currentFrame.setRegion(
						animActual.getKeyFrame(!quieto ? timerAnim += Gdx.graphics.getDeltaTime() : 0, false)); // Conseguir
																												// la
																												// animacion
			else
				currentFrame.setRegion(animActual.getKeyFrames()[0]);
		} catch (Exception e) {
			System.out
					.println("Error accediendo al index: " + (!quieto ? timerAnim += Gdx.graphics.getDeltaTime() : 0));
			System.out.println(
					"animacionActual: " + animActual.getKeyFrames().length + " - " + animActual.getKeyFrames()[0]);
		}
		...
		if (!cameraLock)
			x = (float) (this.body.getX()) + (Juego.parallax) + (Juego.camera.viewportWidth / 2);
		Launcher.batch.draw(currentFrame, (float) (x + (this.body.getWidth() * (this.dir ? 0 : 1))),
				(float) this.body.getY(), currentFrame.getRegionWidth() * 3 * (this.dir ? 1 : -1),
				currentFrame.getRegionHeight() * 3); // Pintar la animacion
		if (borrar)
			Juego.borrar.add(this); // añadirlo a la cola de borrado si no tiene hp.
	}
	...
	public void debug() {
		...
		Juego.shapeRenderer.rect((float) (+this.body.getX() + (Juego.parallax) + (Juego.camera.viewportWidth / 2)),
				(float) this.body.getY(), (float) this.body.getWidth(), (float) this.body.getHeight());
		// System.out.println("x: "+((float)this.body.getX())+" y:
		// "+(float)this.body.getY());
		// System.out.println("x2: "+Juego.players.get(0).body.getX()+" parallax:
		// "+Juego.parallax);
		Juego.shapeRenderer.setColor(Color.BLUE);
		Juego.shapeRenderer.rect((float) (Juego.camera.viewportWidth / 2), (float) Juego.players.get(0).body.getY(),
				(float) Juego.players.get(0).getBody().getWidth(), (float) Juego.players.get(0).getBody().getHeight());

		Juego.shapeRenderer.setColor(Color.GOLD);
		Juego.shapeRenderer.rect(
				(float) ((Juego.camera.viewportWidth / 2) + Juego.players.get(0).getBody().getHitbox()[0]),
				(float) (Juego.players.get(0).getBody().getY()), (float) Juego.players.get(0).getBody().getHitbox()[2],
				(float) Juego.players.get(0).getBody().getHitbox()[3]);
	}
